...
int main() { //main function
	char *arr[] = {"circle.c", "rectangle.c", "square.c", "main.c", NULL};
	int ret;
	for(int i=0; i<4; i++) {
	...
	int n;
	while(waitpid(-1, &n, 0) > 0) {   //wait for all child and check exit status
	...
	ret = fork(); 
	if(ret == 0) {   //creating program.out file
	...
	waitpid(-1, &n, 0); //wait for exit status of program.out file
